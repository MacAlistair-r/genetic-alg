...
Task.prototype.run = function (callback) {
  ...
  async.series([
    ...
    function (cb) { self.elitismStep(cb); },
...
Task.prototype.init = function (callback) {
  ...
  const condition = () => this.population.length === this.popSize;
  var untilBody = (callback) => {
    this.getRandomSolution((solution) => {
  ...
  var untilEnd = (err) => {
    if (err !== null) {
      ...
      self.emit('init end', this.population);
      callback();
    }
  };
  async.until(condition, untilBody, untilEnd);
}
...
Task.prototype.loop = function (callback) {
  ...
  async.until(
    function () { return self.stopCriteria() },
...
Task.prototype.iteration = function (callback) {
  ...
  async.series([
    ...
    function (cb) { self.elitismStep(cb); }, // Apply elitism after generating new offspring
...
Task.prototype.reproduction = function (callback) {
  var self = this, roulette, max, min, avg, sum = 0
  ...
  async.series([
    ...
    function (callback) {
      ...
      async.until(
        ...
        function (cb) {
          var point = Math.random() * sum, position = 0, level = self.parents[position].score, child, parent, secondParent, secondParentPosition
          while (point > level) {
            position++
...
Task.prototype.elitismStep = function (callback) {
  var self = this;
  self.emit('elitism start');
  // Sort population based on whether we are minimizing or maximizing
  self.population.sort(function (a, b) {
    return self.minimize ? a.score - b.score : b.score - a.score;
  });
  // Preserve only the top N elites where N is the `elitism` count
  var elites = self.population.slice(0, self.elitism);
  // Continue to use these elites for the next generation's starting point
  self.population = elites.concat(self.population.slice(self.elitism));
  self.emit('elitism end');
  callback();
};
module.exports = Task